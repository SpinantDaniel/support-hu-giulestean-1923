...
async function edge(name,payload={},allowRefresh=true){
  let s=state.session;
  if(!s?.access_token)s=await currentSession();
  if(!s?.access_token)throw new Error('Sesiunea a expirat. Intră din nou în cont.');

  const request=session=>fetch(`${SUPABASE_URL}/functions/v1/${name}`,{
    method:'POST',
    headers:{
  ...
  if(r.status===401&&allowRefresh){
    try{
      ...
      if(refreshed.data?.session){
        ...
        r=await request(refreshed.data.session);
      }
    }catch(e){
...
function revealAdminShell(){
  ...
  if(badge)badge.textContent=state.role==='admin'?'Administrator full':'Editor newsletter';
...
async function init(){
  try{
    ...
    let access;
    try{
      access=await edge('admin-access',{});
    }catch(e){
      console.error('admin access',e);
      return deny('Nu am putut verifica drepturile contului. Reîncarcă pagina sau autentifică-te din nou.');
    }

    state.role=access?.role||'none';
    if(state.role==='none')return deny('Acest cont nu are acces la panoul de administrare sau newsletter.');

    try{
      const {data:profile,error}=await db.from('profiles').select('display_name').eq('id',state.user.id).maybeSingle();
      if(error)console.warn('admin profile',error);
      state.profile=profile||null;imageService.flushPendingDeletes(db,state.user.id).catch(error=>console.warn('image cleanup retry',error));
    }catch(e){
      console.warn('admin profile network',e);
      state.profile=null;
    }
    ...
    if(state.role==='admin'){
      ...
      const results=await Promise.allSettled([loadDashboard(),loadEditors(),loadPosts()]);
      results.forEach((result,index)=>{
        if(result.status==='rejected')console.error(['dashboard','editors','posts'][index],result.reason);
      });
    }else{
      state.activeTab='newsletter';
      const roleText=$('#newsletterRoleText');
      if(roleText)roleText.textContent='Ai acces editorial limitat: poți publica și edita propriile articole. Nu ai acces la utilizatori, raportări sau anunțuri.';
      showTab(state.activeTab);
      try{await loadPosts();}catch(e){console.error('posts',e);}
    }
  }catch(e){
    console.error('admin init fatal',e);
    deny('Panoul nu s-a putut conecta la sesiunea curentă. Revino în Marketplace, apoi deschide din nou Admin.');
  }
}
...
function bind(){
  ...
  const refresh=$('#refreshDashboard');if(refresh)refresh.onclick=loadDashboard;
  const userSearch=$('#userSearch');if(userSearch)userSearch.oninput=renderUsers;
  const listingSearch=$('#listingSearch');if(listingSearch)listingSearch.oninput=renderListings;
  const editorForm=$('#editorForm');if(editorForm)editorForm.onsubmit=addEditor;
  const postForm=$('#postForm');if(postForm)postForm.onsubmit=savePost;
  const cancel=$('#cancelPostEdit');if(cancel)cancel.onclick=resetPostForm;
  const imageInput=$('#postForm [name=image]');if(imageInput)imageInput.onchange=e=>previewLocalImage(e.target.files?.[0]);
}
...
async function savePost(e){
  e.preventDefault();
  const form=e.currentTarget,btn=$('#savePostBtn'),fd=new FormData(form),editingId=String(fd.get('id')||''),file=form.elements.image.files?.[0];
  setBusy(btn,true,file?'Se optimizează imaginea…':(editingId?'Se salvează…':'Se publică…'));
  let newImagePath=null;
  try{
    const id=editingId||crypto.randomUUID();
    const title=String(fd.get('title')||'').trim(),body=String(fd.get('body')||'').trim(),excerpt=String(fd.get('excerpt')||'').trim()||body.slice(0,240),status=String(fd.get('status')||'published'),oldImage=String(fd.get('image_path')||'')||null;
    let imagePath=oldImage;

    if(file){
      const processed=await imageService.optimize(file,'blog');
      btn.textContent='Se încarcă imaginea…';
      newImagePath=imageService.uniquePath({userId:state.user.id,scopeId:id},processed);
      await imageService.upload(db,{bucket:'blog-images',path:newImagePath,processed});
      imagePath=newImagePath;
      console.info('blog image optimization',{from:imageService.formatBytes(processed.originalSizeBytes),to:imageService.formatBytes(processed.sizeBytes),size:`${processed.width}×${processed.height}`,type:processed.mime,quality:processed.quality});
    }

    const row={author_id:state.user.id,author_name:state.profile?.display_name||state.user.email?.split('@')[0]||'Support Hub',title,excerpt,body,image_path:imagePath,status};
    if(editingId){
      const current=state.posts.find(p=>p.id===editingId);if(!current||!canEditPost(current))throw new Error('Nu poți edita acest articol.');
      const {error}=await db.from('blog_posts').update(row).eq('id',editingId);if(error)throw error;
      if(newImagePath&&oldImage&&oldImage!==newImagePath){
        const cleanup=await imageService.delete(db,'blog-images',[oldImage],{ownerId:state.user.id,retries:2});
        if(!cleanup.ok)console.warn('old blog image cleanup queued',cleanup.error);
      }
    }else{
      row.id=id;row.slug=`${slugify(title)}-${Date.now().toString(36)}`;
      const {error}=await db.from('blog_posts').insert(row);if(error)throw error;
    }

    toast(editingId?'Articol actualizat.':status==='published'?'Articol publicat.':'Ciornă salvată.','success');
    resetPostForm();await loadPosts();
  }catch(err){
    console.error(err);
    if(newImagePath){
      const cleanup=await imageService.delete(db,'blog-images',[newImagePath],{ownerId:state.user?.id,retries:2});
      if(!cleanup.ok)console.warn('failed blog upload cleanup queued',cleanup.error);
    }
    toast(err.message||'Articolul nu a putut fi salvat.','error');
  }finally{setBusy(btn,false);}
}
async function deletePost(id){
  const p=state.posts.find(x=>x.id===id);if(!p||!canEditPost(p))return;
  if(!confirm('Ștergi definitiv acest articol?'))return;
  try{
    const {error}=await db.from('blog_posts').delete().eq('id',id);if(error)throw error;
    let cleanupOk=true;
    if(p.image_path){
      const cleanup=await imageService.delete(db,'blog-images',[p.image_path],{ownerId:state.user.id,retries:2});
      cleanupOk=cleanup.ok;
      if(!cleanup.ok)console.warn('blog delete cleanup queued',cleanup.error);
    }
    toast(cleanupOk?'Articol șters.':'Articol șters. Curățarea imaginii va fi reîncercată automat.','success');
    if($('#postForm [name=id]').value===id)resetPostForm();await loadPosts();
  }catch(e){toast(e.message,'error');}
}
