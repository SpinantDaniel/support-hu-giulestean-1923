...
function setBusy(btn,busy,label='Se procesează…'){if(!btn)return;if(busy){if(!btn.disabled)btn.dataset.label=btn.textContent;btn.disabled=true;btn.textContent=label;}else{btn.disabled=false;btn.textContent=btn.dataset.label||btn.textContent;delete btn.dataset.label;}}
...
async function init(){
  ...
  const {data:profile}=await db.from('profiles').select('display_name').eq('id',state.user.id).maybeSingle();state.profile=profile||null;imageService.flushPendingDeletes(db,state.user.id).catch(error=>console.warn('image cleanup retry',error));
...
function previewLocalImage(file){if(!file){$('#postImagePreview').hidden=true;return;}const u=URL.createObjectURL(file);$('#postImagePreview').hidden=false;$('#postImagePreview').innerHTML=`<img src="${u}" alt="Previzualizare" decoding="async">`;}
function editPost(id){const p=state.posts.find(x=>x.id===id);if(!p||!canEditPost(p))return;const f=$('#postForm');f.elements.id.value=p.id;f.elements.image_path.value=p.image_path||'';f.elements.title.value=p.title||'';f.elements.excerpt.value=p.excerpt||'';f.elements.body.value=p.body||'';f.elements.status.value=p.status||'published';f.elements.image.value='';$('#postFormTitle').textContent='Editează articolul';$('#savePostBtn').textContent='Salvează modificările';$('#cancelPostEdit').hidden=false;if(p.image_path){$('#postImagePreview').hidden=false;$('#postImagePreview').innerHTML=`<img src="${esc(imageUrl(p.image_path))}" alt="${esc(p.title)}" loading="lazy" decoding="async">`;}else $('#postImagePreview').hidden=true;scrollTo({top:0,behavior:'smooth'});}
