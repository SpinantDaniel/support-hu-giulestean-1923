...
Deno.serve(async (req: Request) => {
  ...
  try{
    ...
    if(userError||!user)return json({error:"Sesiunea a expirat."},401);
    ...
    if(listing.seller_id!==user.id)return json({error:"Poți șterge doar propriul anunț."},403);
    ...
    const {data:deleted,error:deleteError}=await admin.from("listings")
      .delete().eq("id",listingId).eq("seller_id",user.id).select("id");
    if(deleteError)throw deleteError;
    if(!deleted?.length)return json({error:"Anunțul nu a putut fi șters."},409);

    return json({ok:true});
  }catch(error){
    console.error("delete-listing-v2",error);
