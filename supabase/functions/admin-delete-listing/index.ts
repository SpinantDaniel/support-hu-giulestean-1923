...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: images, error: imageReadError } = await admin
      .from("listing_images")
      .select("storage_path")
    ...
    if (!deleted?.length) return json({ error: "Anunțul nu mai există." }, 404);
    ...
    if (paths.length) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        if (!rm.error) { cleaned = true; break; }
        console.warn("listing image cleanup", attempt + 1, rm.error.message);
    ...
    console.error("admin-delete-listing", error);
